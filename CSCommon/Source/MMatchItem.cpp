...
bool MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		{
			if (!_stricmp(szAttrValue, MICTOK_TYPE_MELEE))			pNewDesc->m_nType.Set(MMIT_MELEE);
			else if (!_stricmp(szAttrValue, MICTOK_TYPE_RANGE))		pNewDesc->m_nType.Set(MMIT_RANGE);
			else if (!_stricmp(szAttrValue, MICTOK_TYPE_EQUIP))		pNewDesc->m_nType.Set(MMIT_EQUIPMENT);
			else if (!_stricmp(szAttrValue, MICTOK_TYPE_CUSTOM))		pNewDesc->m_nType.Set(MMIT_CUSTOM);
			else if( !_stricmp(szAttrValue, MICTOK_TYPE_TICKET) )	pNewDesc->m_nType.Set(MMIT_TICKET);
			else if( !_stricmp(szAttrValue, MICTOK_TYPE_AVATAR) )	pNewDesc->m_nType.Set(MMIT_AVATAR);
			else if( !_stricmp(szAttrValue, MICTOK_TYPE_COMMUNITY) )	pNewDesc->m_nType.Set(MMIT_COMMUNITY);
			else if( !_stricmp(szAttrValue, MICTOK_TYPE_LONGBUFF) )	pNewDesc->m_nType.Set(MMIT_LONGBUFF);

			pNewDesc->m_nType.MakeCrc();
		}
		else if (!_stricmp(szAttrName, MICTOK_RES_SEX))
		{
			if (!_stricmp(szAttrValue, MICTOK_RES_SEX_M))		pNewDesc->m_nResSex.Set(0);
			else if (!_stricmp(szAttrValue, MICTOK_RES_SEX_F))	pNewDesc->m_nResSex.Set(1);
			else if (!_stricmp(szAttrValue, MICTOK_RES_SEX_A))	pNewDesc->m_nResSex.Set(-1);
		...
		{
			// 슬롯 타입
			if (!_stricmp(szAttrValue, MICTOK_SLOT_NONE))			pNewDesc->m_nSlot = MMIST_NONE;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_MELEE))		pNewDesc->m_nSlot = MMIST_MELEE;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_RANGE))		pNewDesc->m_nSlot = MMIST_RANGE;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_CUSTOM))		pNewDesc->m_nSlot = MMIST_CUSTOM;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_HEAD))		pNewDesc->m_nSlot = MMIST_HEAD;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_CHEST))		pNewDesc->m_nSlot = MMIST_CHEST;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_HANDS))		pNewDesc->m_nSlot = MMIST_HANDS;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_LEGS))		pNewDesc->m_nSlot = MMIST_LEGS;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_FEET))		pNewDesc->m_nSlot = MMIST_FEET;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_FINGER))		pNewDesc->m_nSlot = MMIST_FINGER;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_EXTRA))		pNewDesc->m_nSlot = MMIST_EXTRA;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_AVATAR))		pNewDesc->m_nSlot = MMIST_AVATAR;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_COMMUNITY)) 	pNewDesc->m_nSlot = MMIST_COMMUNITY;
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_LONGBUFF)) 	pNewDesc->m_nSlot = MMIST_LONGBUFF;
		}
		else if (!_stricmp(szAttrName, MICTOK_WEAPON))
		{
			// 무기 타입
			if (strlen(szAttrValue) <= 0)								pNewDesc->m_nWeaponType.Set(MWT_NONE);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_DAGGER))		pNewDesc->m_nWeaponType.Set(MWT_DAGGER);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_DUALDAGGER))	pNewDesc->m_nWeaponType.Set(MWT_DUAL_DAGGER);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_KATANA))		pNewDesc->m_nWeaponType.Set(MWT_KATANA);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_GREATSWORD))	pNewDesc->m_nWeaponType.Set(MWT_GREAT_SWORD);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_DOUBLEKATANA))	pNewDesc->m_nWeaponType.Set(MWT_DOUBLE_KATANA);

			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_PISTOL))		pNewDesc->m_nWeaponType.Set(MWT_PISTOL);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_PISTOLX2))		pNewDesc->m_nWeaponType.Set(MWT_PISTOLx2);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_REVOLVER))		pNewDesc->m_nWeaponType.Set(MWT_REVOLVER);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_REVOLVERX2))	pNewDesc->m_nWeaponType.Set(MWT_REVOLVERx2);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SMG))			pNewDesc->m_nWeaponType.Set(MWT_SMG);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SMGX2))		pNewDesc->m_nWeaponType.Set(MWT_SMGx2);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SHOTGUN))		pNewDesc->m_nWeaponType.Set(MWT_SHOTGUN);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SAWEDSHOTGUN))	pNewDesc->m_nWeaponType.Set(MWT_SAWED_SHOTGUN);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_RIFLE))		pNewDesc->m_nWeaponType.Set(MWT_RIFLE);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_MACHINEGUN))	pNewDesc->m_nWeaponType.Set(MWT_MACHINEGUN);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_ROCKET))		pNewDesc->m_nWeaponType.Set(MWT_ROCKET);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SNIFER))		pNewDesc->m_nWeaponType.Set(MWT_SNIFER);

			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_MEDKIT))		pNewDesc->m_nWeaponType.Set(MWT_MED_KIT);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_REPAIRKIT))	pNewDesc->m_nWeaponType.Set(MWT_REPAIR_KIT);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_BULLETKIT))	pNewDesc->m_nWeaponType.Set(MWT_BULLET_KIT);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_FLASHBANG))	pNewDesc->m_nWeaponType.Set(MWT_FLASH_BANG);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_FRAG))			pNewDesc->m_nWeaponType.Set(MWT_FRAGMENTATION);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SMOKE))		pNewDesc->m_nWeaponType.Set(MWT_SMOKE_GRENADE);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_SKILL))		pNewDesc->m_nWeaponType.Set(MWT_SKILL);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_FOOD))			pNewDesc->m_nWeaponType.Set(MWT_FOOD);

			// enchant
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_ENCHANT_FIRE))			pNewDesc->m_nWeaponType.Set(MWT_ENCHANT_FIRE);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_ENCHANT_COLD))			pNewDesc->m_nWeaponType.Set(MWT_ENCHANT_COLD);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_ENCHANT_LIGHTNING))	pNewDesc->m_nWeaponType.Set(MWT_ENCHANT_LIGHTNING);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_ENCHANT_POISION))		pNewDesc->m_nWeaponType.Set(MWT_ENCHANT_POISON);

			// 소모성
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_POTION))			pNewDesc->m_nWeaponType.Set(MWT_POTION);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_TRAP))				pNewDesc->m_nWeaponType.Set(MWT_TRAP);
			else if (!_stricmp(szAttrValue, MICTOK_WEAPON_DYNAMITE))			pNewDesc->m_nWeaponType.Set(MWT_DYNAMITYE);
			

			pNewDesc->m_nWeaponType.MakeCrc();
		}		
		else if (!_stricmp(szAttrName, MICTOK_SPEND_TYPE))
		{
			if (strlen(szAttrValue) <= 0)									pNewDesc->m_nSpendType.Set(MMCT_NONE);
			else if (!_stricmp(szAttrValue, MICTOK_SPEND_TYPE_NORMAL))		pNewDesc->m_nSpendType.Set(MMCT_NORMAL);
			else if (!_stricmp(szAttrValue, MICTOK_SPEND_TYPE_LONGBUFF))		pNewDesc->m_nSpendType.Set(MMCT_LONGBUFF);
			else if (!_stricmp(szAttrValue, MICTOK_SPEND_TYPE_SHORTBUFF))	pNewDesc->m_nSpendType.Set(MMCT_SHORTBUFF);
			else if (!_stricmp(szAttrValue, MICTOK_SPEND_TYPE_COMMUNITY))	pNewDesc->m_nSpendType.Set(MMCT_COMMUNITY);
		...
		else if (!_stricmp(szAttrName, MICTOK_ITEMPOWER))
		{
			pNewDesc->m_nItemPower.Set( atoi(szAttrValue));
			pNewDesc->m_nItemPower.MakeCrc();
		}
		else if (!_stricmp(szAttrName, MICTOK_DAMAGE_TYPE))
		{
			if (strlen(szAttrValue) <= 0)									pNewDesc->m_nDamageType.Set(MMDT_NORMAL);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_FIRE))		pNewDesc->m_nDamageType.Set(MMDT_FIRE);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_COLD))		pNewDesc->m_nDamageType.Set(MMDT_COLD);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_POISION))		pNewDesc->m_nDamageType.Set(MMDT_POISION);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_LIGHTNING))	pNewDesc->m_nDamageType.Set(MMDT_LIGHTING);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_HEAL))		pNewDesc->m_nDamageType.Set(MMDT_HEAL);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_REPAIR))		pNewDesc->m_nDamageType.Set(MMDT_REPAIR);
			else if (!_stricmp(szAttrValue, MICTOK_DAMAGE_TYPE_HASTE))		pNewDesc->m_nDamageType.Set(MMDT_HASTE);

			pNewDesc->m_nDamageType.MakeCrc();
		}
		else if (!_stricmp(szAttrName, MICTOK_DAMAGE_TIME))
		{
			pNewDesc->m_nDamageTime.Set( atoi(szAttrValue));
			pNewDesc->m_nDamageTime.MakeCrc();
		}
		else if (!_stricmp(szAttrName, MICTOK_LIFETIME))
		{
		...
		{
			if (!_stricmp(szAttrValue, MICTOK_TRUE)) 	pNewDesc->m_bIsCashItem = true;
		...
		{
			if (!_stricmp(szAttrValue, MICTOK_TRUE)) pNewDesc->m_bSlugOutput = true;
		...
		else if (!_stricmp(szAttrName, MICTOK_ANGLE))
		{
		...
		else if (!_stricmp(szAttrName, MICTOK_SOUND_WEAPON_FIBER))
		{
		...
		else if( !_stricmp(szAttrName, MICTOK_BONUS_SPENDABLE) )
		{
			if( 0 == _stricmp(MICTOK_FALSE, szAttrValue) )	pNewDesc->m_bIsSpendableItem = false;
			else											pNewDesc->m_bIsSpendableItem = true;
		}
		else if( !_stricmp(szAttrName, MICTOK_TICKET_TYPE) )
		{
			if( 0 == _stricmp(szAttrValue, MICTOK_TICKET_TYPE_ADMISSION) )
				pNewDesc->m_TicketType = MMTT_ADMISSION;
			else if( 0 == _stricmp(szAttrValue, MICTOK_TICKET_TYPE_CHANGEHEAD) )
				pNewDesc->m_TicketType = MMIT_CHANGEHEAD;
			else {
				ASSERT( 0 && "머야? 이 종이는 머야?" );
			}
		}
		else if (!_stricmp(szAttrName, MICTOK_MAX_RENT_PERIOD))
		{
		...
		else if (!_stricmp(szAttrName, MICTOK_AVATAR_HEAD))
		{
		...
		else if (!_stricmp(szAttrName, MICTOK_AVATAR_CHEST))
		{
			strcpy(pNewDesc->m_pAvatarMeshName->Ref().m_szChestMeshName
				, MGetStringResManager()->GetStringFromXml(szAttrValue));
			pNewDesc->m_pAvatarMeshName->MakeCrc();
		}
		else if (!_stricmp(szAttrName, MICTOK_AVATAR_HAND))
		{
			strcpy(pNewDesc->m_pAvatarMeshName->Ref().m_szHandMeshName
				, MGetStringResManager()->GetStringFromXml(szAttrValue));
			pNewDesc->m_pAvatarMeshName->MakeCrc();			
		}
		else if (!_stricmp(szAttrName, MICTOK_AVATAR_LEGS))
		{
			strcpy(pNewDesc->m_pAvatarMeshName->Ref().m_szLegsMeshName
				, MGetStringResManager()->GetStringFromXml(szAttrValue));
			pNewDesc->m_pAvatarMeshName->MakeCrc();
		}
		else if (!_stricmp(szAttrName, MICTOK_AVATAR_FEET))
		{
