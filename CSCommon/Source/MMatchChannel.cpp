						...
						const bool bIsTicketChannel, const DWORD dwTicketItemID, const bool bIsUseTicket, const char* pszNameStrResId)
{
	...
	if (nType == MCHANNEL_TYPE_PRESET || nType == MCHANNEL_TYPE_DUELTOURNAMENT)
	{
		...
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_CHAMPION_STR))		m_nRuleType = MCHANNEL_RULE_CHAMPION;
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_QUEST_STR))			m_nRuleType = MCHANNEL_RULE_QUEST;
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_DUELTOURNAMENT_STR))	m_nRuleType = MCHANNEL_RULE_DUELTOURNAMENT;
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_SET1_STR))			m_nRuleType = MCHANNEL_RULE_SET1;
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_SET2_STR))			m_nRuleType = MCHANNEL_RULE_SET2;
		else if (!_stricmp(m_szRuleName, MCHANNEL_RULE_SET3_STR))			m_nRuleType = MCHANNEL_RULE_SET3;
	}
