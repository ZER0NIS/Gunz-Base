...
{
	const DWORD ADDVERTEX_INTERVAL = 17;
...
{
	if (false == IsTimeToAddVertex()) return;
...
HRESULT RFireEffectTexture::Create(LPDIRECT3DDEVICE9 dev, int w, int h)
{
	...
	RtlZeroMemory(m_pData, w * h);
	RtlZeroMemory(m_pData2, w * h);

	m_pFireActive = m_pData;
	m_pFireScratch = m_pData2;

	if (FAILED(hr = D3DXCreateTexture(dev, w, h, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED, MakeWriteProxy(m_pTexture))))
		return hr;
...
HRESULT RFireEffectTexture::UpdateTexture()
{
	...
	RtlZeroMemory(&lockedrect, sizeof(lockedrect));
	...
	for (y = 0; y < m_h; y++) {
		for (x = 0; x < m_w; x++) {
			...
			pSurfBits[index++] = m_Palette[m_pFireActive[pos]].peBlue;
			pSurfBits[index++] = m_Palette[m_pFireActive[pos]].peGreen;
			pSurfBits[index++] = m_Palette[m_pFireActive[pos]].peRed;
			pSurfBits[index++] = m_Palette[m_pFireActive[pos]].peFlags;
		}
