...
bool MZip::InitializeImpl()
{
	...
	if (m_nZipMode >= ZMode_Mrs2)
		ZERONIS(reinterpret_cast<char*>(&dh), sizeof(MZIPDIRHEADER));
	...
	if (m_nZipMode >= ZMode_Mrs2)
		ZERONIS((char*)m_pDirData, dh.dirSize);
...
size_t MZip::GetFileArchiveOffset(int i)
{
	...
	if (m_nZipMode >= ZMode_Mrs2)
		ZERONIS(reinterpret_cast<char*>(&h), sizeof(h));
...
bool MZip::ReadFile(int i, void* pBuffer, int nMaxSize)
{
	...
	const auto OutputSize = min(static_cast<dword>(nMaxSize), h.ucSize);
...
bool MZip::RecoveryMrs2(FILE* fp)
{
	...
	ZERONIS( (char*)pDirData , dir_data_size );//mrs ¶ó¸é º¯È¯..
	...
	for (int i = 0; i < dh.nDirEntries; i++) {
		...
		ZERONIS( _fileheaderReader , _fileheaderReaderSize );
...
bool MZip::RecoveryZip(char* zip_name)
{
	...
	// mrs1 ÀÎÁö ½Äº°..ÀÌ¹Ì ÆÄÀÏµéÀÌ ³ª°£»óÅÂ¿©¼­ ½Äº°¹æ¹ýÀÌ ÀÌ°Í»Ó...

	if( isVersion1Mrs(fp) ) {	// ÃÖÃÊ¸ðµ¨...
		RecoveryMrs( fp );
	}
	else {
		RecoveryMrs2( fp );		// v2 ºÎÅÍ´Â Çì´õÀÇ sig °ªÀ¸·Î ¹öÀü±¸ºÐ....
	}
